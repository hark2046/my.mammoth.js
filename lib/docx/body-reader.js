...
function BodyReader(options) {
    ...
    function readFldChar(element) {
        ...
        if (type === "begin") {
            const children = readChildElements(element);
            // currentElementsInFldChar.push(...children.value);
            complexFieldStack.push(...children.value);
            currentInstrText = [];
        } else if (type === "end") {
            // console.log(JSON.stringify(currentElementsInFldChar));
            let formFieldItem = complexFieldStack[0];
            if(formFieldItem.formItemType === 'textInput') {
                const valueItems =  complexFieldStack.filter(item=>item.type==='text').map(t => t.value);
                if (valueItems && valueItems.length > 0){
                    formFieldItem = {...complexFieldStack[0], value: valueItems.join(' ')};
                }

            }
            complexFieldStack = [];
            return formFieldItem ? elementResult(formFieldItem) : emptyResult();
    ...
    var xmlElementReaders = {
        ...
        "w:t": function(element) {
            if(complexFieldStack.length > 0) {
                complexFieldStack.push(new documents.Text(element.text()))
            }
        ...
        "w:ffData": function(element) {
            countOfFfData += 1;
            // console.log(`第${countOfFfData}次找到ffData`)
            if (element.children && element.children.length > 0) {
                const option = {};
                const name = element.first('w:name').attributes['w:val'];
                const control = element.first('w:textInput') || element.first('w:checkBox') || element.first('w:ddList');
                if (!control) {
                    return emptyResult();
                }
                const formItemType = control.name.replace('w:', '');
                option.name = name;
                option.formItemType = formItemType;
                if (formItemType === 'checkBox') {
                    const valueElement = control.first('w:checked');
                    if(_.isUndefined(valueElement.attributes['w:val'])) {
                        option.value = '1'
                    } else if (`${valueElement.attributes['w:val']}` === '0') {
                        option.value = '0';
                    } else {
                        option.value = '-1'
                    }
                }
                if (formItemType === 'ddList') {
                    const valueElement = control.first('w:result');
                    const valueIndex = valueElement ? parseInt(`${valueElement.attributes['w:val']}`, 10) : 0;
                    const optionsList = control.children.filter(item => item.name === 'w:listEntry').map(entry => entry.attributes['w:val']);
                    option.value = `${optionsList[valueIndex]}`;
                }

                return elementResult(new documents.FormField(option))
            }
            return emptyResult();
        },
